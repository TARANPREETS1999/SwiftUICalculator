...
//  SwiftUICalculator
//
//  Created by Taranpreet Singh on 27/12/21.
//

import SwiftUI

struct ContentView: View {
    let grid = [["AC", "x", "%", "/"], ["7", "8", "9", "*"], ["4", "5", "6", "-"], ["1", "2", "3", "+"], [".", "0", "", "="]]
    let operators = ["/", "+", "*", "%"]
    ...
    var body: some View {
        ...
        .alert(isPresented: $showAlert) {
            Alert(
                title: Text("Invalide Input"), message: Text(visibleWorkings),
    ...
    func calculateResults() -> String {
        ...
        workings = visibleWorkings.replacingOccurrences(of: "*", with: "X")
        let expression = NSExpression(format: workings)
        let results = expression.expressionValue(with: nil, context: nil) as! Double
        return formatResults(val: results)
        }
        showAlert = true
        return ""
    }
